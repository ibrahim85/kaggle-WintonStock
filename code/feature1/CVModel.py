...
def creatCVIndex():
    ...
    X_train,X_test,Y_train,Y_test=train_test_split(train,train['Id'],test_size=0.5,random_state=200)
    dumpCVIndex("../../data/cv/train.run1.txt", list(Y_train-1))
    dumpCVIndex("../../data/cv/valid.run1.txt", list(Y_test-1))
    X_train,X_test,Y_train,Y_test=train_test_split(train,train['Id'],test_size=0.5,random_state=825)
